...
export class DashboardComponent implements OnInit {
  ...
  ngOnInit() {
    this.statsService.getStats().subscribe((stats: StatsDTO) => {
      ...
      this.animateCounter('totalPatients', stats.totalPatients, () => this.updateTotalPatientsChart());
      this.animateCounter('activePatients', stats.totalPatients - stats.deletedPatients, () => this.updateTotalPatientsChart());
      this.animateCounter('deletedPatients', stats.deletedPatients, () => this.updateTotalPatientsChart());
      this.animateCounter('femalePatients', stats.femaleCount, () => this.updateGenderChart());
      this.animateCounter('malePatients', stats.maleCount, () => this.updateGenderChart());
  ...
  animateCounter(property: keyof this, targetValue: number, callback?: () => void) {
    const duration = 500; 
        ...
        datasets: [
          {
            ...
            hoverBackgroundColor: [gradientAverage, gradientMale, gradientFemale]
          }
        ]
      };
    }
        ...
        datasets: [
          {
            ...
            hoverBackgroundColor: [gradientIstanbul, gradientAnkara, gradientIzmir]
          }
        ]
      };
    }
  ...
  updateTopThreeCitiesChart(patientsByCity: { [city: string]: number }) {
    const sortedCities = Object.entries(patientsByCity).sort((a, b) => b[1] - a[1]).slice(0, 3);
