...
import { ComboBoxService } from '../services/combo-box.service';  // Service to interact with the database
...
export class ComboBoxComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.loadCities();
    this.formGroup.get(this.cityControlName)?.valueChanges.subscribe(cityId => {
      if (cityId) {
  ...
  loadCities() {
    this.comboBoxService.getCities().subscribe(cities => {
      this.cities = cities;
    }, error => {
