...
export class PatientDetailsModalComponent implements OnInit, OnChanges {
  ...
  initializeFormGroup(): void {
    ...
    // React to city changes to load corresponding counties
  ...
  loadCityAndCounty(): void {
    if (this.selectedPatient) {
      // Get the city ID associated with the patient's residence (county ID)
      this.comboBoxService.getCityIdByCountyId(this.selectedPatient.residence).subscribe((cityId) => {
  ...
  loadCounties(cityId: number): void {
    this.comboBoxService.getCounties(cityId).subscribe((counties) => {
      ...
      // Ensure the county is set if it exists
      if (this.selectedPatient) {
        this.formGroup.patchValue({
          residenceCounty: this.selectedPatient.residence
        });
  ...
  save() {
    // Update the selected patient with the county ID from the formGroup
    if (this.selectedPatient) {
      this.selectedPatient.residence = this.formGroup.get('residenceCounty')?.value;
    }
