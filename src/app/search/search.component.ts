...
@Component({
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, InputTextModule, DropdownModule, ButtonModule, NavigateHomeButtonComponent],
  selector: 'app-search',
...
export class SearchComponent implements OnInit {
  ...
  @Output() search = new EventEmitter<{ searchBy: string, searchTerm: string }>();
  ...
  onSearch() {
    if (this.searchForm.valid) {
      ...
      if (searchBy === 'nameSurname') {
        ...
        this.search.emit({ searchBy: 'nameSurname', searchTerm: JSON.stringify({ firstName, lastName }) });
      } else {
